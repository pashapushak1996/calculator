...
const App = () => {
    ...
    return (
        <div className={'container d-flex dir-column align-center'}>
            ...
            <div>
                <button className={'mx-10 btn'} value={1}
                        onClick={({target: {value}}) => handleCounter(+value, dispatch)}>1
                </button>
                <button className={'mx-10 btn'} value={-1}
                        onClick={({target: {value}}) => handleCounter(+value, dispatch)}>-1
                </button>
            ...
            <div className={'mx-10'}>
                <button className={'mx-10 btn'} value={100}
                        onClick={({target: {value}}) => handleCounter(+value, dispatch)}>100
                </button>
                <button className={'mx-10 btn'} value={-100}
                        onClick={({target: {value}}) => handleCounter(+value, dispatch)}>-100
                </button>
            ...
            <div className={'d-flex mx-10 align-center'}>
                ...
                <input type="text" onInput={({target: {value}}) => setMyInputValue(value)} value={myInputValue}/>
